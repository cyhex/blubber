...
public class TaskServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException,
        ...
        List<Task> tasks = DbService.ofy().load().type(Task.class)
                .filter("status ==", Task.TaskStatus.CLOSED).list();

        tasks.addAll(DbService.ofy().load().type(Task.class)
                .filter("status ==", Task.TaskStatus.OPEN).list());

        tasks.addAll(DbService.ofy().load().type(Task.class)
                .filter("status ==", Task.TaskStatus.CONFIRMED).list());
